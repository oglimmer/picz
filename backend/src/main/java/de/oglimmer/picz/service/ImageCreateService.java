...
public class ImageCreateService {
  ...
  private void extractFile() {
    try (InputStream inputStream =
            ImageCreateService.class.getResourceAsStream("/" + OUTOFSPACE_JPG);
        OutputStream outputStream =
            new FileOutputStream(Path.of(appConfig.getImagePath(), OUTOFSPACE_JPG).toFile())) {
      ByteStreams.copy(inputStream, outputStream);
    } catch (IOException e) {
      log.error("Error copying file", e);
    }
    try (InputStream inputStream =
            ImageCreateService.class.getResourceAsStream("/" + OUTOFSPACE_SMALL_JPG);
        OutputStream outputStream =
            new FileOutputStream(
                Path.of(appConfig.getSmallImagePath(), OUTOFSPACE_SMALL_JPG).toFile())) {
      ByteStreams.copy(inputStream, outputStream);
    } catch (IOException e) {
      log.error("Error copying file", e);
    }
  ...
  public UploadedImage create(MultipartFile image, long albumId) {
    ...
    uploadedImage.setConvertedFileExtension(
        "heic".equals(ext) ? "jpg" : ext); // this needs to be here and not in
    // uploadedImageToAlbumService.createImages(uploadedImage);
    ...
    if (checkForDuplicatedHash(uploadedImage)) {
      ...
      temporaryFileService.save(
          uploadedImage.getFilename(),
          Files.readAllBytes(uploadedImage.getOriginalImagePath(appConfig)));
  ...
  private void saveFileToDisk(MultipartFile image, UploadedImage uploadedImage) throws IOException {
    ...
    Files.copy(
        image.getInputStream(), outputPath, java.nio.file.StandardCopyOption.REPLACE_EXISTING);
  }
}
