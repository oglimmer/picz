...
public class AlbumRepositoryService {
    ...
    public AlbumElement updateMap(long albumId, Long imageId, Double mapCenterLongitude, Double mapCenterLatitude, Double mapSpanLongitude, Double mapSpanLatitude, Double longitude, Double latitude) {
        AlbumElement image = albumElementRepository.findById(imageId).orElseThrow();
        ...
        updateMapElement(image, mapCenterLongitude, mapCenterLatitude, mapSpanLongitude, mapSpanLatitude, longitude, latitude);
        return image;
    }

    public AlbumElement updateGoogleMap(long albumId, Long imageId, Double mapCenterLatitude, Double mapCenterLongitude,
                                        Double markerLatitude, Double markerLongitude, Integer zoomLevel) {
        AlbumElement image = albumElementRepository.findById(imageId).orElseThrow();
        ...
        updateMapElement(image, mapCenterLatitude, mapCenterLongitude, markerLatitude, markerLongitude, zoomLevel);
        return image;
    }
    ...
    private void createMapElement(AlbumElement currentElement) {
        ...
        if (previousIndex >= 0 && previousIndex < albumElements.size()) {
            ...
            newOrderNo = (currentElement.getOrderNo() - previousElement.getOrderNo()) / 2 + previousElement.getOrderNo();
        } else {
        ...
        if (currentElement.getLongitude() != null && currentElement.getLatitude() != null) {
            ...
            Optional<AlbumElement> first = album.getAlbumElements().stream().filter(e -> e.getLongitude() != null && e.getLatitude() != null).findFirst();
            if (first.isPresent()) {
        ...
        imageResizeService.resize(imagePath,
                smallPath,
                ImageCreateService.PREVIEW_IMAGE_WIDTH, album.getUser().getJpgQuality());
    ...
    @SneakyThrows
    private void updateMapElement(AlbumElement dateElement, Double mapCenterLongitude, Double mapCenterLatitude, Double mapSpanLongitude, Double mapSpanLatitude, Double longitude, Double latitude) {
        albumElementFileDeletionService.deleteFromPersistentStorage(dateElement);
        Album album = dateElement.getAlbum();
        String filename = RandomData.generateRandomString(32) + ".png";
        Path imagePath = Paths.get(appConfig.getImagePath() + filename);
        byte[] image = mapAppleGenerator.generateMap(latitude, longitude, mapCenterLatitude, mapCenterLongitude, mapSpanLatitude, mapSpanLongitude);
        Files.write(imagePath, image);
    ...
    @SneakyThrows
    private void updateMapElement(AlbumElement dateElement, Double mapCenterLatitude, Double mapCenterLongitude, Double markerLatitude, Double markerLongitude, Integer zoomLevel) {
        albumElementFileDeletionService.deleteFromPersistentStorage(dateElement);
        Album album = dateElement.getAlbum();
        String filename = RandomData.generateRandomString(32) + ".png";
        Path imagePath = Paths.get(appConfig.getImagePath() + filename);
        byte[] image = mapGoogleGenerator.generateMap(mapCenterLatitude, mapCenterLongitude, markerLatitude, markerLongitude, zoomLevel);
        Files.write(imagePath, image);
    ...
    private AlbumElement getAlbumElement(Album album, long orderNo) {
        ...
        dateElement.setAlbum(album);
        albumElementRepository.save(dateElement);
        log.debug("created date element {} for album:{} with orderNo {}", dateElement.getId(), album.getId(), orderNo);
        return dateElement;
    }
    ...
    public void deleteElement(long id, long elementId) {
        ...
        if (ae.getElementType() == AlbumElementType.IMAGE || ae.getElementType() == AlbumElementType.MAP) {
    ...
    public void reorder(long id, int oldOrder, int newOrder) {
        ...
        if (newOrder < oldOrder) {
            newOrderNo = newOrder > 0 ? (target.getOrderNo() + albumElements.get(newOrder - 1).getOrderNo()) / 2 : target.getOrderNo() - 1000;
        } else {
            newOrderNo = newOrder < albumElements.size() - 1 ? (albumElements.get(newOrder + 1).getOrderNo() + target.getOrderNo()) / 2 : target.getOrderNo() + 1000;
        }
        source.setOrderNo(newOrderNo);
    }

    public Album findBySecretId(String secretId) {
        return albumRepository.findBySecretId(secretId).orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Album " + secretId + " not found"));
    }
    ...
    public AlbumElement findAlbumElementBySecretId(String id) {
        ...
        return albumElementRepository.findBySecretId(id).orElseThrow(() -> new RuntimeException("AlbumElement " + id + " not found"));
    }
    ...
    public void createSectionIfNeeded(Long imageElementId) {
        AlbumElement imageEntity = albumElementRepository.findById(imageElementId)
                .orElseThrow(() -> new RuntimeException("ImageEntity not found for id: " + imageElementId));
    ...
    public void deleteAlbum(long id) {
        ...
        albumElementRepository.deleteAllById(album.getAlbumElements().stream().mapToLong(AlbumElement::getId).boxed().toList());
