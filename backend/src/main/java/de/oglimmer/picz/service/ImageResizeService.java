...
public class ImageResizeService {
    ...
    @SneakyThrows
    public void resize(Path sourceImagePath, Path destinationImagePath, int targetWidth, float jpgQuality) {
        ...
        try {
            ...
            int targetHeight = (int) ((double) sourceImage.getHeight() / sourceImage.getWidth() * targetWidth);
            // Create a new resized image
            BufferedImage resizedImage = new BufferedImage(targetWidth, targetHeight, BufferedImage.TYPE_INT_RGB);
            try {
                // Resize the source image to the target dimensions
                resizedImage.getGraphics().drawImage(sourceImage.getScaledInstance(targetWidth, targetHeight, BufferedImage.SCALE_SMOOTH), 0, 0, null);
    ...
    private static void writeImage(BufferedImage resizedImage, File output, float quality) throws IOException {
    ...
    public void removeExif(Path sourceImagePath, Path destinationImagePath, float jpgQuality) {
        ...
        try {
            ...
            BufferedImage resizedImage = new BufferedImage(sourceImage.getWidth(), sourceImage.getHeight(), BufferedImage.TYPE_INT_RGB);
            try {
    ...
    private BufferedImage initImage(Path sourceImagePath) throws IOException {
        ...
        AffineTransformOp op = new AffineTransformOp(getExifTransformation(imageInformation), AffineTransformOp.TYPE_BICUBIC);
