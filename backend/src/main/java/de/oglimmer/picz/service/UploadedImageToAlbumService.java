...
public class UploadedImageToAlbumService {
    ...
    public AlbumElement createImage(UploadedImage uploadedImage) {
        ...
        if (!uploadedImage.getOriginalImagePath(appConfig).toFile().exists()) {
            byte[] fileContent = temporaryFileService.findByUniqueId(uploadedImage.getFilename()).orElseThrow(() ->
                    new RuntimeException("File not found: " + uploadedImage.getOriginalImagePath(appConfig)));
        ...
        imageEntity.setFilename(uploadedImage.getFilename() + "." + uploadedImage.getConvertedFileExtension());
        ...
        if (user.getMaxImageWidth() == 0) {
            ...
            imageResizeService.resize(originalImagePath,
                    imagePath,
                    user.getMaxImageWidth(), user.getJpgQuality());
        }
    ...
    private void convertToJpeg(UploadedImage uploadedImage) {
        ...
        String outputFile = Paths.get(appConfig.getOriginalImagePath() + uploadedImage.getFilename() + "." + uploadedImage.getConvertedFileExtension()).toString();
    ...
    public static String formatDate(AlbumElement uploadedImage) {
        ...
        return (uploadedImage.getCreationDate() != null ? simpleDateFormat.format(uploadedImage.getCreationDate())
                : "null") + " / " + simpleDateFormat.format(new Date(uploadedImage.getOrderNo()));
    }
