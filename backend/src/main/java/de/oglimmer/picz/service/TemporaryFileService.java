...
public class TemporaryFileService {
    ...
    public void save(String uniqueId, byte[] content) {
        try {
            ...
            log.warn("Failed to save file {} to REST API, falling back to database: {}", uniqueId, e.getMessage());
    ...
    public Optional<byte[]> findByUniqueId(String uniqueId) {
        try {
            ...
            log.warn("Failed to load file {} from REST API, falling back to database: {}", uniqueId, e.getMessage());
    ...
    public void deleteByUniqueId(String uniqueId) {
        try {
            ...
            log.warn("Failed to delete file {} from REST API, falling back to database: {}", uniqueId, e.getMessage());
        }
    ...
    private void saveToDatabase(String uniqueId, byte[] content) {
        TemporaryFiles temporaryFiles = TemporaryFiles.builder()
                .uniqueId(uniqueId)
                .content(content)
                .build();
    ...
    private Optional<byte[]> loadFromDatabase(String uniqueId) {
        return temporaryFilesRepository.findByUniqueId(uniqueId)
                .map(TemporaryFiles::getContent);
    }
