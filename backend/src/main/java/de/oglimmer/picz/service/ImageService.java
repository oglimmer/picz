...
public class ImageService {
    ...
    public byte[] get(AlbumElement albumElement, boolean small) {
        Path readPath = imageStorage.loadFromPersistent(albumElement.getFilename(), small, albumElement.getAlbum().getUser());
        if (readPath == null) {
            readPath = Paths.get(small ? appConfig.getSmallImagePath() + OUTOFSPACE_SMALL_JPG : appConfig.getImagePath() + OUTOFSPACE_JPG);
        }
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    imageResizeService.resize(outputFile.toPath(), Path.of(appConfig.getSmallImagePath(), image.getFilename()), ImageCreateService.PREVIEW_IMAGE_WIDTH, image.getAlbum().getUser().getJpgQuality());
