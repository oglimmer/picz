...
@Service
@Slf4j
@ConditionalOnProperty(value = "picz.imageStorageImplementation", havingValue = "ImageStorageS3")
public class ImageStorageS3 implements ImageStorage {
  ...
  public void deleteCache() {
    ...
    for (Path dir :
        Arrays.asList(
            Paths.get(appConfig.getImagePath()), Paths.get(appConfig.getSmallImagePath()))) {
      try {
                  ...
                  path -> {
                    try {
                      return !path.getFileName()
                              .toString()
                              .equals(ImageCreateService.OUTOFSPACE_JPG)
  ...
  public long deletePersistentImagePath(String filename) {
    ...
    long size =
        s3Client
            .headObject(builder -> builder.bucket(appConfig.getS3Bucket()).key(objectPath))
            .contentLength();
  ...
  public void transferToPersistentStorage(Path imagePath, Path smallPath) {
    s3Client.putObject(
        builder ->
            builder
                .bucket(appConfig.getS3Bucket())
                .key(appConfig.getS3ImagePath() + imagePath.getFileName().toString())
                .build(),
        imagePath);
    s3Client.putObject(
        builder ->
            builder
                .bucket(appConfig.getS3Bucket())
                .key(appConfig.getS3SmallImagePath() + smallPath.getFileName().toString())
                .build(),
        smallPath);
  }
  ...
  public Path loadFromPersistent(String filename, boolean small, User user) {
    ...
    if (!path.toFile().exists()) {
      try {
        ...
        s3Client.getObject(
            builder ->
                builder.bucket(appConfig.getS3Bucket()).key(appConfig.getS3ImagePath() + filename),
            imagePath);
        ...
        s3Client.getObject(
            builder ->
                builder
                    .bucket(appConfig.getS3Bucket())
                    .key(appConfig.getS3SmallImagePath() + filename),
            smallImagePath);
      } catch (NoSuchKeyException e) {
