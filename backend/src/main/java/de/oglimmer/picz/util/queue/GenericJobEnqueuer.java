...
/**
 * A service to enqueue “bean.method(...)” jobs. Internally serializes MethodInvocationPayload to
 * JSON and stores it in job_queue.
 */
@AllArgsConstructor
@Service
...
public class GenericJobEnqueuer {
   ...
   * @param processorName which scheduler should process this job (MAIN_SCHEDULER or
   *     PARALLEL_SCHEDULER)
   */
  public void enqueue(
      String beanName,
    ...
    MethodInvocationPayload payload =
        new MethodInvocationPayload(beanName, methodName, parameterTypeNames, args);
