...
public abstract class AbstractJobScheduler {
    ...
    protected boolean claimJob(JobQueue job) {
        int updated = jobQueueRepository.updateJobStatusIfMatch(
                job.getId(),
    ...
    public String processJobInTransaction(String payloadJson) {
        MethodInvocationPayload payload = objectMapper.readValue(payloadJson, MethodInvocationPayload.class);
...
//            method.setAccessible(true);
    ...
    protected void onFailed(Throwable throwable, JobQueue job) {
        onSuccess(job, JobQueue.JobStatus.FAILED, new JobResult(job, null, getStackTraceAsString(throwable)));
    }
