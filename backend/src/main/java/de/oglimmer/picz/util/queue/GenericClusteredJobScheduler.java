...
/**
 * This scheduler executes jobs a single-threaded manner, ensuring that only one instance of the job is active at a time across the cluster.
 */
@Slf4j
@Component
public class GenericClusteredJobScheduler extends AbstractJobScheduler {
    ...
    public GenericClusteredJobScheduler(JobQueueRepository jobQueueRepository,
    ...
    protected void processScheduledJobs() {
        ...
        do {
            for (JobQueue job : dueJobs) {
                if (claimJob(job)) {
                    try {
                        AbstractJobScheduler jobScheduler = applicationContext.getBean("genericClusteredJobScheduler", AbstractJobScheduler.class);
