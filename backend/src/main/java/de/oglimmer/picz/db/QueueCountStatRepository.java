...
public interface QueueCountStatRepository extends CrudRepository<QueueCountStat, Long> {
  ...
  @Query(
      value =
          "INSERT INTO queue_count_stats (album_id, user_id, processing_count)"
              + "     VALUES (:albumId, :userId, 1)"
              + "  ON DUPLICATE KEY UPDATE processing_count = processing_count + 1",
      nativeQuery = true)
  void upsertIncrementProcessingCount(@Param("albumId") long albumId, @Param("userId") long userId);

  /**
   * Decrement processing_count by 1, but only if processing_count > 1.
   *
   * <p>Returns the number of rows actually updated (0 or 1).
   */
  @Modifying
  @Query(
  ...
  int decrementIfGreaterThanOne(@Param("albumId") long albumId, @Param("userId") long userId);

  /**
   * Delete the row, but only if processing_count == 1.
   *
   * <p>Returns the number of rows actually deleted (0 or 1).
   */
  @Modifying
  @Query(
      "DELETE FROM QueueCountStat q"
          ...
          + "   AND q.userId  = :userId"
          + "   AND q.processingCount = 1")
  int deleteIfCountEqualsOne(@Param("albumId") long albumId, @Param("userId") long userId);
