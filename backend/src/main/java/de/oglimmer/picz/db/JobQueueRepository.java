...
public interface JobQueueRepository extends JpaRepository<JobQueue, Long> {
  ...
  @Transactional(propagation = Propagation.REQUIRES_NEW)
  @Query(
      "SELECT j FROM JobQueue j WHERE j.nextRun <= :now AND j.status = :status AND j.processorName = :processorName "
          + "ORDER BY j.nextRun ASC")
  List<JobQueue> findDueJobs(
  ...
  @Modifying
  @Query(
      "UPDATE JobQueue j SET j.status = :status, j.lockedBy = :lockedBy, j.lockedAt = :lockedAt "
