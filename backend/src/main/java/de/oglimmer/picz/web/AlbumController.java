...
public class AlbumController {
    ...
    @PatchMapping("/{id}")
    public void patchAlbum(@RequestBody PatchAlbumRequest patchAlbumRequest, @PathVariable("id") long id) {
    ...
    public void createSection(@RequestBody CreateAlbumSectionRequest createAlbumSectionRequest,
    ...
    public void createMap(@RequestBody CreateAlbumMapRequest createAlbumMapRequest, @PathVariable("id") long id) {
    ...
    public AlbumElementResponse updateMap(@RequestBody UpdateAlbumMapRequest updateAlbumMapRequest, @PathVariable("id") long id) {
        AlbumElement albumElement = albumRepositoryService.updateMap(id, updateAlbumMapRequest.getImageId(), updateAlbumMapRequest.getMapCenterLongitude(),
                updateAlbumMapRequest.getMapCenterLatitude(), updateAlbumMapRequest.getMapSpanLongitude(),
                updateAlbumMapRequest.getMapSpanLatitude(), updateAlbumMapRequest.getLongitude(), updateAlbumMapRequest.getLatitude());
    ...
    public AlbumElementResponse updateMapGoogle(@RequestBody UpdateAlbumGoogleMapRequest updateAlbumMapRequest, @PathVariable("id") long id) {
        AlbumElement albumElement = albumRepositoryService.updateGoogleMap(id, updateAlbumMapRequest.getImageId(),
                updateAlbumMapRequest.getMapCenterLatitude(), updateAlbumMapRequest.getMapCenterLongitude(),
                updateAlbumMapRequest.getMarkerLatitude(), updateAlbumMapRequest.getMarkerLongitude(),
    ...
    public void reorder(@RequestBody OrderAlbumRequest orderAlbumRequest, @PathVariable("id") long id) {
        albumRepositoryService.reorder(id, orderAlbumRequest.getOldIndex(), orderAlbumRequest.getNewIndex());
    }
    ...
    public void deleteElement(@PathVariable("id") long id, @PathVariable("elementId") long elementId) {
        albumRepositoryService.deleteElement(id, elementId);
    }

    @Operation(
            summary = "Deletes an entire album",
            security = @SecurityRequirement(name = "OpenID")
    )
    ...
    public ResponseEntity<List<ListAlbumResponse>> list() {
        ...
        List<Album> allList = StreamSupport
                .stream(all.spliterator(), false)
                .toList();
        List<ListAlbumResponse> list = allList.stream()
                .map(album -> modelMapper.map(album, ListAlbumResponse.class))
                .collect(Collectors.toList());
        list.forEach(e -> e.setOwner(StreamSupport.stream(all.spliterator(), false)
                .filter(f -> Objects.equals(f.getId(), e.getId())).findAny().orElseThrow().getUser().getEmail()));
        if (user.isAdmin()) {
            list.forEach(e -> e.setDescription(e.getDescription() + " (" + e.getOwner() + ")"));
        }
        list.forEach(e -> e.setTitleSecretId(allList.stream()
                ...
                .findAny()
                .map(AlbumElement::getSecretId).orElse(null)));
        list.forEach(e -> e.setImageCount(allList.stream()
                .filter(f -> f.getId().equals(e.getId()))
                .findAny()
                .orElseThrow()
                .getAlbumElements()
                .stream().filter(g -> g.getElementType() == AlbumElementType.IMAGE).count()));
