    ...
    public void migrate() {
        try (ExecutorService executor = Executors.newFixedThreadPool(4)) {
            try (S3Client s3Client = S3Client.builder()
                ...
                List<CompletableFuture<Void>> futures = allElements.stream()
                        .filter(e -> e.getFilename() != null && !e.getFilename().isBlank())
                        .flatMap(e -> Stream.of(
                                CompletableFuture.runAsync(() -> {
                                    Path imagePath = Path.of(appConfig.getImagePath(), e.getFilename());
                                    s3Client.putObject(
                                            req -> req.bucket(appConfig.getS3Bucket())
                                                    .key(appConfig.getS3ImagePath() + imagePath.getFileName()),
                                            imagePath
                                    );
                                }, executor),
                                CompletableFuture.runAsync(() -> {
                                    Path smallImagePath = Path.of(appConfig.getSmallImagePath(), e.getFilename());
                                    s3Client.putObject(
                                            req -> req.bucket(appConfig.getS3Bucket())
                                                    .key(appConfig.getS3SmallImagePath() + smallImagePath.getFileName()),
                                            smallImagePath
                                    );
                                }, executor)
                        ))
