...
public class UserController {
  ...
  public GetUserResponse getUser() {
    ...
    List<CountElementByAlbum> countElementByAlbum =
        albumRepository.findCountElementByAlbum(user.getId(), AlbumElementType.IMAGE);

    response.setProcessingAlbumIds(albumListInUploadByUser);
    response.setProcessingCounterByAlbum(
        queueCountStats.getProcessingCounterByAlbum(albumListInUploadByUser));
    response.setNumberOfImagesByAlbum(
        countElementByAlbum.stream()
            .collect(
                java.util.stream.Collectors.toMap(
                    CountElementByAlbum::getId, CountElementByAlbum::getCount)));
  ...
  @Operation(
      summary =
          "Deletes the currently authenticated user and all of the associated albums and images",
      security = @SecurityRequirement(name = "OpenID"))
  @DeleteMapping
